...
def allocate_buffers(engine: trt.ICudaEngine, profile_idx: Optional[int] = None):
    ...
    for binding in tensor_names:
        ...
        #print(engine.get_tensor_profile_shape(binding, profile_idx), binding)
    ...
    return inputs, outputs, output_shapes, output_names, bindings, stream, inputs_shape
...
class TrtModel(object):
    ...
    def build(self):
        ...
        self.inputs, self.outputs, self.output_shapes, self.output_names, self.bindings, self.stream, self.inputs_shape = allocate_buffers(self.engine, 0)
    ...
    def run(self, input=None, deflatten: bool = True, as_dict=False, from_device=False, infer_shape=None):
        self.inputs[0].host = input.ravel()
        self.context.set_input_shape('input', input.shape)
        assert self.context.all_binding_shapes_specified

        trt_outputs = do_inference(self.context, engine=self.engine, bindings=self.bindings,
                                        inputs=self.inputs, outputs=self.outputs, stream=self.stream)
        trt_outputs = [output.reshape(shape) for output, shape in zip(trt_outputs, self.output_shapes)]

        trt_outputs = {name: trt_outputs[i][:input.shape[0]] for i, name in enumerate(self.output_names)}
